...
IconUp(TwmWindow * tmp_win)
{
  ...
  {
    if (!XGetGeometry(dpy, tmp_win->icon_w.win, &JunkRoot, &defx, &defy, &JunkWidth, &JunkHeight, &JunkBW, &JunkDepth))
      return;
...
GetIconImage(char *name, Pixel background, unsigned int *numcolors)
{
  ...
  if (iconimage == NULL)
  {
    ...
    if (bm != None)
    {
      ...
      XGetGeometry(dpy, bm, &JunkRoot, &JunkX, &JunkY, &JunkWidth, &JunkHeight, &JunkBW, &JunkDepth);
    ...
    if (iconimage != NULL)
      AddToList(&Scr->Icons, name, LTYPE_EXACT_NAME, (char *)iconimage);
  }
...
CreateIconWindow(TwmWindow * tmp_win, int def_x, int def_y)
{
  ...
  if (Scr->IconBevelWidth > 0 && !Scr->BeNiceToColormap)
    GetShadeColors(&tmp_win->iconc);
  ...
  if (Scr->ForceIcon)
  {
    ...
    if (icon_name != NULL)
      iconimage = GetIconImage(icon_name, tmp_win->iconc.back, &pm_numcolors);
  ...
  if (pm == None && tmp_win->wmhints && tmp_win->wmhints->flags & IconPixmapHint)
  {
    XGetGeometry(dpy, tmp_win->wmhints->icon_pixmap, &JunkRoot, &JunkX, &JunkY, &JunkWidth, &JunkHeight, &JunkBW, &JunkDepth);
    ...
    iconimage->pixmap = XCreatePixmap(dpy, Scr->Root, iconimage->width, iconimage->height, Scr->d_depth);

    if (JunkDepth == Scr->d_depth)
      XCopyArea(dpy, tmp_win->wmhints->icon_pixmap, iconimage->pixmap,
		Scr->NormalGC, 0, 0, iconimage->width, iconimage->height, 0, 0);
    else
      XCopyPlane(dpy, tmp_win->wmhints->icon_pixmap, iconimage->pixmap,
		 Scr->NormalGC, 0, 0, iconimage->width, iconimage->height, 0, 0, 1);
  ...
  if (tmp_win->wmhints && tmp_win->wmhints->flags & IconWindowHint)
  {
	...
	XGetGeometry(dpy, tmp_win->icon_w.win, &JunkRoot, &JunkX, &JunkY,
		     (unsigned int *)&tmp_win->icon_w_width, (unsigned int *)&tmp_win->icon_w_height, &JunkBW, &JunkDepth) == 0)
    {
  ...
  XSelectInput(dpy, tmp_win->icon_w.win, KeyPressMask | ButtonPressMask | ButtonReleaseMask | event_mask);

  tmp_win->icon_bm_w = None;
  if (pm != None && (!(tmp_win->wmhints && tmp_win->wmhints->flags & IconWindowHint)))
  {
    int y;
  ...
  if (final_x > Scr->MyDisplayWidth)
    final_x = Scr->MyDisplayWidth - tmp_win->icon_w_width - (2 * Scr->IconBorderWidth);

  if (final_y > Scr->MyDisplayHeight)
    final_y = Scr->MyDisplayHeight - tmp_win->icon_height - Scr->IconFont.height - 4 - (2 * Scr->IconBorderWidth);
